...
**Why passive-interface on access VLANs:** If you don't set passive-interface on the VLAN SVIs, a workstation could theoretically send OSPF hellos and get a neighbor adjacency. Passive-interface stops OSPF hellos from going out those interfaces while still advertising the subnet.

**Why Rapid PVST+ and BPDU Guard:** Standard 802.1D takes 30-50 seconds to converge after a topology change. Rapid PVST+ does it in under 2 seconds. BPDU Guard shuts down any access port that receives a BPDU, which stops someone from plugging in a rogue switch.
...
2. 2. Set up trunks between access switches and SW-DIST
   3. 3. Enable ip routing on SW-DIST, configure SVIs, bring up the routed link to R1-CORE
      4. 4. Configure OSPF on both R1-CORE and SW-DIST
         5. 5. Add the GUEST-RESTRICT ACL on SW-DIST Vlan30
            6. 6. SSH harden all devices (generate RSA keys, restrict VTY to SSH, set banners)
               7. 7. Deploy pfSense, configure LAN rules and NAT
                  8. 8. Add SNMPv3 and syslog forwarding on all Cisco devices
                    
                     9. The config files in /configs have the full running configs for each device. Comments in the files explain why things are configured the way they are.
                    
                     10. ## Verification
                    
                     11. After building, the key things to verify:
                     ...
                     12. ```
                         ...
                         ```
                         configs/          running configs for all four Cisco devices
                         pfsense/          pfSense firewall rules and NAT setup
                         docs/             ITIL change log and troubleshooting guide
                         verification/     test results and expected show command outputs
                         ```
